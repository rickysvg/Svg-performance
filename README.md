...
Headings, lime highlighter kickers, primary pills, and big stats use **Anton** (`font-display`). Body copy stays Geist. Primary tabs (phone, sticky top, lime highlight on the active tab): **Home · Train · Fuel · Learn · Coach**. Shop, Book, Profile, and Log out sit on the bottom account bar with the official circular neon badge. First open plays Ricky’s splash video until the neon ring closes (muted so phones actually animate; skipped on reduced motion and later visits). The + button is a quick add for workout, food, a body metric, or heart rate. Paid app plans are **additional to gym dues**.
...
The first deploy has empty tables (no DEMO program until you seed). Home still loads — it shows an empty training CTA instead of “This page hit a snag.” Seed when you want the DEMO **strength** days, DEMO **martial-arts skill** days, and lessons:
...
Train / Home / Calendar use a **Core weekday planner** on top of those seeded days. **Weighted lifts log reps + lbs.** **Loaded carries (farmer / suitcase / rack) log seconds + lbs** (`load_timed`). Skill days are timed rounds; planks / cardio are timed; chin-ups and band work are reps only. Experience on the profile scales round length and rest (beginner ~2 min / 90s rest; advanced/pro ~3–5 min / 30–45s). After this logging-mode change lands, **re-seed Neon** so hosted rows get the correct `logMode` tags:
