...
- Each `ProgramExercise` has a stored `logMode` (`load_reps`, `reps_only`, `timed`, `timed_round`). Name heuristics are fallback only for old rows. Planks and other holds log **seconds**, not lbs. DEMO Combat Skills bag / pads / grappling blocks log **timed rounds**. The logger hides weight on those modes and labels rest as “between rounds” when it is a skill round.
