...
- **Core weekday planner** (`src/lib/week-plan.ts`) is the source of truth for Home / Train / Calendar — not the old single alternating pick. Average members share one Mon–Sun skeleton (bag+strength on Mon/Wed, lighter skill or recovery on Tue, grappling or second striking on Thu, conditioning+legs on Fri, optional Sat, rest Sun). `primaryFocus` picks the skill flavor; general fitness gets strength/conditioning only (no bag). Availability + `sessionsPerWeek` hide days (Saturday first). Labeled DEMO / Core — not Elite, injury, or fight-camp customization (those stay out of scope).
...
- Each `ProgramExercise` has a stored `logMode` (`load_reps`, `reps_only`, `timed`, `timed_round`). **`load_reps` is only for weighted lifts** (dumbbell / barbell / kettlebell / cable / machine load). Bodyweight count work is `reps_only` (no lbs). Holds, cardio, mobility, and unknown non-lifts are `timed`. DEMO Combat Skills bag / pads / grappling blocks stay `timed_round`. Name heuristics default to **timed**, not lbs. The logger hides weight unless `load_reps`.
...
- App-open splash (`AppSplash` in the root layout) plays Ricky’s official badge video (`public/svg-performance-splash.mp4`) on first visit: square 960 clip, trimmed to the moment the neon streak closes the ring (~8.15s). Playback starts **muted** (`muted` + `autoPlay` + `playsInline`) so phones actually animate; sound is optional and never gates motion. No poster/still card during video (that square outline was the completed-logo still on a different black). The splash field is `#000` and the clip is slightly overscanned so edges blend. Dismisses when the clip ends and the app is ready, then writes `sessionStorage.svg_splash_seen`. `prefers-reduced-motion` shows the closed-ring still with a short fade — not the video. White shell + `#CBF805` + black stay. Header chrome still uses the official circular badge.
