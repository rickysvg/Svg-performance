...
- Each `ProgramExercise` has a stored `logMode` (`load_reps`, `load_timed`, `reps_only`, `timed`, `timed_round`). **`load_reps` is only for weighted lifts** (dumbbell / barbell / kettlebell / cable / machine load). **`load_timed` is loaded carries / weighted holds** (farmer, suitcase, overhead/rack carry) — lbs + seconds, no reps. Bodyweight count work is `reps_only` (no lbs). Unloaded holds, cardio, mobility, and unknown non-lifts are `timed`. DEMO Combat Skills bag / pads / grappling blocks stay `timed_round`. Name heuristics map `carry` / `farmer` to `load_timed` before the lift rule, then default to **timed**. The logger hides weight unless `load_reps` or `load_timed`. `logMode` is a string field, not a Prisma enum.
